...
<script>
    ...
    export default {
        ...
        methods: {
            clearMemory() {
                Object.assign(this.$data, this.$options.data()) // Faz com que o objeto volte ao estado inicial 
            },
            setOperation(operation) {
                if (this.current === 0) {
                    ...
                    const _currentOperation = this.operation

                    try {
                        this.values[0] = eval(
                            `${this.values[0]} ${_currentOperation} ${this.values[1]}`
                        )
                    } catch (e) {
                        this.$emit('onError', e)
                    }
